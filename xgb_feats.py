...
if __name__ == "__main__":
    ...
    train_X = pd.read_csv(data_path + 'train_feats_v2.csv', usecols=columns + ['label'])
    ...
    test_X = pd.read_csv(data_path + 'test_feats_v2.csv', usecols=columns)
    # for col in ['sum', 'renta', 'canel', 'lag5', 'com20', 'sum8']:
    for col in ['come175']:
    ...
    if cv_sel == 1:
        ...
        nfolds = 10
        kf = KFold(n_splits=nfolds, shuffle=True, random_state=seed_val)
        preds = [0] * NUM_CLASS
    ...
    out_df.to_csv('./submit/sub_xgb_feats.csv', index=False)
